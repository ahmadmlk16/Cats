...
/*
 ******************************
 MARK: - Read Cat Data File
 ******************************
 */
public func readCatDataFile() {
   
    let catDataFilename = "CatData.json"
   
    // Obtain URL of the cat data file in document directory on user's device
    let urlOfJsonFileInDocumentDirectory = documentDirectory.appendingPathComponent(catDataFilename)
 
    do {
        _ = try Data(contentsOf: urlOfJsonFileInDocumentDirectory)
       
        // Cat data file exists in the document directory
 
        catStructList = loadFromDocumentDirectory(catDataFilename)
        print("atData is loaded from document directory")
       
    } catch {
        // Cat data file does not exist in the document directory; Load it from the main bundle.
       
        catStructList = loadFromMainBundle(catDataFilename)
        print("CatData is loaded from main bundle")
    }
}
 
/*
***********************************************
MARK: - Load Cat Data File from Main Bundle
***********************************************
*/
...
/*
 *****************************************************
 MARK: - Write Cat Data File to Document Directory
 *****************************************************
 */
public func writeCatDataFile() {
 
    // Obtain the URL of the JSON file in the document directory on the user's device
    let urlOfJsonFileInDocumentDirectory: URL? = documentDirectory.appendingPathComponent("CatData.json")
 
    let encoder = JSONEncoder()
    if let encoded = try? encoder.encode(catStructList) {
        do {
            try encoded.write(to: urlOfJsonFileInDocumentDirectory!)
        } catch {
            fatalError("Unable to write encoded cat data to document directory!")
        }
    } else {
        fatalError("Unable to encode cat data!")
    }
}
 
/*
******************************************************
MARK: - Load Cat Data File from Document Directory
******************************************************
*/
...
/*
***************************************
MARK: - Get Cat Flag Image from URL
***************************************
